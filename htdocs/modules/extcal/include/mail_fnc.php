...
// exit;
  ...
  if ($mode == _EXTCAL_HEADER_HTML){
    $template = 'extcal_mail_member_html.html';
  }else{
    $template = 'extcal_mail_member_text.html';
  }
  $mail_body = $tpl->fetch('db:' . $template);
  
  extcal_SendMail($destinataires,
                  $mail_fromName,
                  $mail_fromemail,
                  $mail_subject,
                  $mail_body,
 ...
 *****************************************************************/
function extcal_SendMail($destinataires,
                    $mail_fromname,
                    $mail_fromemail,
                    $mail_subject,
                    $mail_body,
...
global  $ModName, $signature, $mail_admin, $xoopsConfig, $xoopsDB, $xoopsModule;
  ...
  if ($mail_fromname=='') $mail_fromname= $mail_fromemail;   
  
  //$mail_fromemail = "admin@win-trading.com";  
  //$mail_subject = "test hemes";
  //$mail_body = getContentTestMail(); 
  //-----------------------------
  if (!is_array($destinataires)) $destinataires= explode($sep, $destinataires);
