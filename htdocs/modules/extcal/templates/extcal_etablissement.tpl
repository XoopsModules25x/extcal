        ...
        <td colspan="3" class="odd" style="padding-right:30px; padding-top:10px;">
            ...
            <div style="font-size:16px; font-weight:bold; width:280px; overflow:hidden; margin-left:30px;">
                ...
                <div style="font-size:14px; ">
                    ...
                    <{if $etablissement.cp}><{$etablissement.cp}><{/if}> - <{if $etablissement.ville}><{$etablissement.ville}><br/><{/if}>
                    <{if $etablissement.map!=''}>
                        <a href="<{$etablissement.map}>" target="_blank"><{$smarty.const._MD_EXTCAL_ETABLISSEMENT_MAP2}></a>
                        <br/>
                    <{/if}>
                    ...
                    <{if $etablissement.site}><a href="<{$etablissement.site}>" target="_blank"><{$smarty.const._MD_EXTCAL_VISIT_SITE}></a><br/><{/if}>
                </div>
            </div>
        </td>
    </tr>
                                        ...
                                        <tr>
                                            <td width="100px" align="center">
                                                <{if $event.event_picture1}>
                                                    <a id="<{$event.event_id}>" class="highslide" onclick="return hs.expand(this)" href="<{$xoops_url}>/uploads/extcal/<{$event.event_picture1}>"><img align=left style="border:1px solid #333333;margin-right:6px" src="<{$xoops_url}>/uploads/extcal/<{$event.event_picture1}>" width="100px" height="100px" /></a>
                                                <{elseif $smarty.const._EXTCAL_SHOW_NO_PICTURE}>
                                                    <img align=left  style="border:1px solid #333333;margin-right:6px" src="<{$xoops_url}>/modules/extcal/assets/images/no_picture.png" width="100px" height="100px" />
                                                <{/if}>
                                            </td>
                                            <td><u><strong><a href="./event.php?event=<{$event.event_id}>"><{$event.event_title}></a></strong></u>&nbsp;&nbsp;( <{$event.event_start}> )<br /><br /><{$event.event_desc}></td>
                                        </tr>
                                    </table>
                                    </div>
                            <{/foreach}>
...
<p style="text-align:right;">
    <{foreach item=eventFile from=$event_attachement}>
        <a href="download_attachement.php?file=<{$eventFile.file_id}>"><{$eventFile.file_nicename}> (<i><{$eventFile.file_mimetype}></i>) <{$eventFile.formated_file_size}></a>
        <br/>
    <{/foreach}>
</p><br/>
