...
if ($xoopsUser && $xoopsModuleConfig['whos_going']) {
    ...
    if ((int)$_POST['event'] > 0 && ($_POST['mode'] === 'add' || $_POST['mode'] === 'remove')) {
        ...
        if ($_POST['mode'] === 'add') {
            ...
            if ($event->getVar('event_nbmember') > 0 && $eventMemberHandler->getNbMember((int)$_POST['event']) >= $event->getVar('event_nbmember')) {
                ...
                $eventMemberHandler->createEventmember(array('event_id' => (int)$_POST['event'], 'uid' => $xoopsUser->getVar('uid')));
